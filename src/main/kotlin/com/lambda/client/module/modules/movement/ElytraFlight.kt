...
object ElytraFlight : Module(
    ...
    /* Fireworks */
    private val fireworkUseMode by setting("Firework Use Mode", FireworkUseMode.SPEED, { mode == Mode.FIREWORKS && page == Page.MODE_SETTINGS })
    private val delay by setting("Fireworks Delay Ticks", 30, 0..100, 1, { mode == Mode.FIREWORKS && fireworkUseMode == FireworkUseMode.DELAY && page == Page.MODE_SETTINGS })
    private val fireworkUseStartSpeed by setting ("Fireworks Use Min Speed", 1.0, 0.01..3.0, 0.01, { mode == Mode.FIREWORKS && fireworkUseMode == FireworkUseMode.SPEED && page == Page.MODE_SETTINGS })
    private val fireworkBlockAvoid by setting("Avoid Blocks", false, { mode == Mode.FIREWORKS && page == Page.MODE_SETTINGS },
        description = "Don't use fireworks if player is facing into a block")
    private val fireworkBlockAvoidDist by setting("Avoid Blocks Raytrace Distance", 2.0, 1.0..10.0, 0.1, { mode == Mode.FIREWORKS && page == Page.MODE_SETTINGS && fireworkBlockAvoid })
    private val fireworksVControl by setting("Boosted V Control", true, { mode == Mode.FIREWORKS && page == Page.MODE_SETTINGS })
    private val fireworksVSpeed by setting("Boosted V Control Speed", 1.0, 0.0..3.0, 0.1, { mode == Mode.FIREWORKS && fireworksVControl && page == Page.MODE_SETTINGS })
